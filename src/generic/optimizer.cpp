...
void Optimizer::setConstruction(const string &constr)
{
    if (constr == "greedy")
        ...
        this->construction = [this]
        { constructRandomReplicate(); };
    else if (constr == "nearestNeighbor")
        this->construction = [this]
        { constructNearestNeighbor(); };
    else
...
#pragma omp parallel for default(none) private(new_fitness, new_penalties) shared(best_solution, perm, freq, updated, best_improved)
    for (uint position = 0; position <= perm.size(); position++)
    { // for all positions i in X
        vector<uint> new_perm = perm;
        vector<uint> new_freq = freq;
        if (this->stop() || (best_improved && config["first_improve"]))
            continue;
...
#pragma omp parallel for default(none) shared(best_solution, best_improved, updated) private(cand_fitness, cand_penalties) firstprivate(cand_perm, cand_freq)
    ...
    { // for all nodes i in A
        ...
        oprtr::append(cand_perm, cand_freq, this->instance->ubs, i);
...
#pragma omp critical
        {
            ...
            if ((cand_penalty < best_penalty) || (cand_penalty == best_penalty && cand_fitness < best_solution.fitness))
            {
                ...
                updated = true;
                if (cand_fitness < best_solution.fitness)
                    best_improved = true;
            }
        };
        oprtr::remove(cand_perm, cand_freq, this->instance->lbs, cand_perm.size() - 1);
    }
...
bool Optimizer::remove1()
{
    ...
    if (this->current_solution.permutation.empty())
        return false;
    vector<uint> perm = this->current_solution.permutation;
    vector<uint> freq = this->current_solution.frequency;
    fitness_t fitness;
...
#pragma omp parallel for default(none) private(fitness, penalties) shared(best_solution, perm, updated)
    for (uint i = 0; i < perm.size(); i++)
    {
        ...
        oprtr::remove(new_perm, new_freq, this->instance->lbs, i);
...
#pragma omp critical
        ...
        oprtr::insert(new_perm, new_freq, this->instance->ubs, node, i);
    }
    if (updated) this->current_solution = best_solution;
...
#pragma omp parallel for default(none) private(fitness, penalties) shared(best_solution, perm, x, reverse, updated)
    for (uint i = 0; i < perm.size(); i++)
    {
        if (this->stop() || (updated && config["first_improve"]))
            continue;
        vector<uint> new_perm = perm;
        for (uint j = 0; j < new_perm.size(); j++)
        {
            oprtr::relocate(new_perm, i, j, x, reverse);
            this->instance->computeFitness(new_perm, fitness, penalties);
...
#pragma omp critical
            ...
            oprtr::relocate(new_perm, j, i, x, reverse);
        }
    }
...
#pragma omp parallel for default(none) private(fitness, penalties) shared(best_solution, perm, x, updated)
    for (int i = (int)x; i < (int)perm.size() - (int)x; i++)
    {
        if (this->stop() || (updated && config["first_improve"]))
            continue;
        vector<uint> new_perm = perm;
        oprtr::centeredExchange(new_perm, i, x);
        this->instance->computeFitness(new_perm, fitness, penalties);
#pragma omp critical
        if (fitness < best_solution.fitness)
        {
...
#pragma omp parallel for default(none) private(fitness, penalties) shared(best_solution, perm, x, y, reverse, updated)
    for (uint i = 0; i < perm.size() - x; i++)
    {
        if (this->stop() || (updated && config["first_improve"]))
            continue;
        vector<uint> new_perm = perm;
        for (uint j = 0; j < new_perm.size() - y; j++)
        {
            if ((i >= j && i < j + y) || (j >= i && j < i + x)) continue;        
            oprtr::exchange(new_perm, i, j, x, y, reverse);
            this->instance->computeFitness(new_perm, fitness, penalties);
#pragma omp critical
            if (fitness < best_solution.fitness)
            {
            ...
            uint undo_i = i > j ? i + x-y : i;
            uint undo_j = j > i ? j + y-x : j;
            oprtr::exchange(new_perm, undo_i, undo_j, y, x, reverse);
        }
    }
...
#pragma omp parallel for default(none) private(fitness, penalties) shared(best_solution, perm, updated)
    ...
    { // for all nodes i
        ...
        for (uint j = 0; j < i; j++)
        { // for all nodes j
            oprtr::exchangeIds(new_perm, new_freq, this->instance->lbs, this->instance->ubs, i,j);
            this->instance->computeFitness(new_perm, fitness, penalties);
#pragma omp critical
            if (fitness < best_solution.fitness)
            {
...
#pragma omp parallel for default(none) private(fitness, penalties) shared(best_solution, perm, updated)
    ...
    { // for all nodes i in A
        ...
        vector<uint> new_freq = freq;
        for (uint j = 0; j < i; j++)
        { // for all pairs of nodes i,j in A
            for (uint n = 1; n < this->current_solution.frequency[i]; n++)
            { // for all frequencies up to f_i
                oprtr::exchangeNIds(new_perm, new_freq, this->instance->lbs, this->instance->ubs, i, j, n);
                this->instance->computeFitness(new_perm, fitness, penalties);
#pragma omp critical
                if (fitness < best_solution.fitness)
                {
...
void Optimizer::doubleBridge(uint k, bool reverse_all)
{
    ...
    if (k < 1)
        throw std::out_of_range("Double bridge: k < 1");
...
{
    std::cout << str(format("-----> [%1%s]") % this->getRuntime()) << "  Current: ["
              << "fitness: " << this->current_solution.fitness << "  updated: " << update << "  feasible: " << this->current_solution.is_feasible << "  size: " << this->current_solution.permutation.size() << "]    BKS: ["
              << "fitness: " << this->best_known_solution.fitness << "  last updated: " << std::chrono::duration_cast<std::chrono::seconds>(this->last_improvement - this->start).count() << "s"
              << "  feasible: " << this->best_known_solution.is_feasible << "  size: " << this->best_known_solution.permutation.size() << "]" << std::endl;
}
